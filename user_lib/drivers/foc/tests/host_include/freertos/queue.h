...
struct host_static_queue
{
    ...
    UBaseType_t queue_length = 0U;
    UBaseType_t item_size = 0U;
    bool has_item = false;
};
...
{
    inline std::atomic<uint32_t> task_overwrite_calls{0U};
    inline std::atomic<uint32_t> task_peek_calls{0U};
    inline std::atomic<uint32_t> isr_overwrite_calls{0U};
    inline std::atomic<uint32_t> isr_peek_calls{0U};
}
...
{
    if(queue_length != 1U || item_size == 0U || !queue_storage ||
        !queue_control)
    {
        return nullptr;
    }
    ...
    TickType_t)
{
    ...
    if(queue->has_item && queue->queue_length == 1U){return 0;}
    ...
    const void *item)
{
    ...
    foc_host_queue::task_overwrite_calls.fetch_add(1U);
    std::lock_guard<std::mutex> lock(queue->mutex);
    std::memcpy(queue->storage, item, queue->item_size);
    queue->has_item = true;
    return pdPASS;
    ...
    TickType_t)
{
    ...
    foc_host_queue::task_peek_calls.fetch_add(1U);
    std::lock_guard<std::mutex> lock(queue->mutex);
    if(!queue->has_item){return 0;}
    std::memcpy(item, queue->storage, queue->item_size);
    return pdPASS;
    ...
    BaseType_t *)
{
    ...
    foc_host_queue::isr_overwrite_calls.fetch_add(1U);
    std::lock_guard<std::mutex> lock(queue->mutex);
    std::memcpy(queue->storage, item, queue->item_size);
    queue->has_item = true;
    return pdPASS;
    ...
    void *item)
{
    ...
    foc_host_queue::isr_peek_calls.fetch_add(1U);
    std::lock_guard<std::mutex> lock(queue->mutex);
    if(!queue->has_item){return 0;}
    std::memcpy(item, queue->storage, queue->item_size);
    return pdPASS;
